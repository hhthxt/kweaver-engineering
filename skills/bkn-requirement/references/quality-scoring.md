...
- `bkn_creator_handoff` 未先按场景做四层收敛，或未区分“业务已确认内容”“仍属建模候选”“需下游建模阶段判定的问题”；
- `bkn_creator_handoff` 不是中文业务可读摘要；
- P0/P1 场景正文未先完成业务化四层收敛，只在文末 handoff 中补对象 / 关系 / 动作清单；
- 四层摘要只列名词，没有说明为何该场景需要这些对象、联系、业务逻辑或治理控制；
- 概念模型层把属性、枚举、计算结果直接误写成对象，或未显式区分对象候选、属性候选、枚举候选和结果候选；
- 动力层把计算、判断、状态变化、Action 混写，未说明哪类逻辑在驱动业务；
- 治理层把查看、刷新、导出等普通交互动作直接写成治理要求，未体现权限、审批、留痕或拒绝自动化边界；
- 四层候选项缺少 `scenario_id`、`evidence_ref` 或 `confirmation_status`；
- “业务已确认内容”中包含“范围待确认”、工程推断或无法追溯到场景 / 规则 / 验收用例的内容。
...
- §13 的场景主追问被字段、接口、写回、SLA 等系统确认问题占据，未保留业务目标、规则、异常和验收主线；
- P0/P1 场景缺少本体分类结论，导致下游无法判断哪些是对象、属性、枚举、关系、结果、逻辑或 Action 候选；
- P0/P1 场景缺少明确决策点、操作闭环或可衡量的业务能力提升，只停留在功能描述；
- P0/P1 场景缺少场景级四层候选摘要；
- 场景级收敛缺少证据引用或确认状态。
...
| 41-60 | 有场景级摘要，但四层不完整或“业务已确认内容”混入待确认内容。 |
| 61-80 | P0/P1 场景正文已完成业务化四层收敛，并有中文业务可读交接摘要；仍有少量边界问题。 |
| 81-100 | 每个 P0/P1 场景都能从业务目标、规则和验收自然推导出四层内容，且有中文可读摘要、证据、确认状态和明确 BKN_Creator 路由。 |
