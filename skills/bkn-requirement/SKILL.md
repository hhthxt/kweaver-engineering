...
7. **生成 BKN_Creator 交接摘要**
   ...
   - 判定准则见 `references/bkn-requirement-method.md` 的“本体论判别纪律”和 `references/bkn-ontology-modeling-method.md` 的“本体建模门禁”。若分类、对象边界、关系成立条件、逻辑归属或行动边界尚不确定，必须标为“待下游建模判定”，不得含混处理。
   ...
   - 不能把候选对象、关系、逻辑属性、结果候选或 Action 伪装成业务已确认事实。
...
- BKN_Creator 交接摘要是否通过本体建模门禁：对象具备身份、生命周期、责任、关系或管理动作；关系连接两个已说明对象；逻辑说明输入、规则、输出和边界；Action 确实改变业务事实或触发受控副作用；治理说明责任、权限、审批、留痕或拒绝自动化边界；
...
- BKN 本体建模方法：`references/bkn-ontology-modeling-method.md`，生成场景收敛小结和 `BKN_Creator` 交接摘要前必读，用于规范对象、属性 / 枚举、关系、逻辑、Action、治理和 Agent 任务的判定与表达，防止字段化、报表化、界面化建模。
